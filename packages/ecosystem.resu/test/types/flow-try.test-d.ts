...
expectType<RuntimeError | ResultError<'Failure', number>>(
	FlowTrySync(() => ({ status: 'error', tag: failureTag, data: 1 }) as ResultError<'Failure', number>),
)
...
expectError(FlowTrySync(async () => 1 as number))
expectError(FlowTrySync({ try: async () => 1 as number }))
